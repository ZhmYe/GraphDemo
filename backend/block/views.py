...
def defaultTx_view(request):
    index = int(request.GET.get("index")) % len(transactions)
    select_transaction = []
    if index < 3:
        select_number = 3 - index
        for i in range(select_number):
            select_transaction.append(transactions[len(transactions) - (select_number - i)])
    for i in range(index + 1):
        select_transaction.append(transactions[i])
    return JsonResponse({
        'message': 'ok',
        "transactions": [{
